...
# ---------------------------------------------------------------------------
# Artifact tool'ları BURADAN KALDIRILDI (2026-09-04)
#
# Artık `services/artifact_service/` altında kendi pod'unda; sandbox oraya
# doğrudan, akışlı HTTP ile konuşuyor. İki gerekçe:
#
#   1. **Taşıma.** Baytlar MCP çağrısının içinde base64 gidiyordu — %33 şişme
#      ve iki uçta tam tampon. 100 MiB sınırı da aslında bunun doğal tavanıydı.
#   2. **Ayrıştırma.** Bu tek pod üç işi birden yapıyordu: dış tool proxy'si,
#      artifact kayıt defteri, MinIO kimlik bilgisi taşıyıcısı. Araştırma
#      dokümanının §26'sı tam olarak bunu "önerilmeyen yaklaşım" diye
#      işaretliyor.
#
# GÜVENLİK KAZANCI: bu pod'un artık MinIO'ya rotası YOK (bkz.
# k8s/policies/tool-gateway-egress — `app: minio` kuralı kaldırıldı). Tool
# Gateway ele geçirilse bile artifact deposuna ulaşılamıyor; Artifact Service
# ele geçirilse bile internete çıkılamıyor. OpenAI'nin Ağustos 2026 olayından
# çıkan "tek workload compromise'ı internet erişimine dönüşmemeli" ilkesi.
# ---------------------------------------------------------------------------
